...
export default function PatientsManagePage() {
  ...
  const handleEdit = (patient: Patient) => {
    setFormData({
      ...
      birthDate: patient.birthDate.split('T')[0], // 只取日期部分
  ...
  const confirmDelete = async () => {
    ...
    try {
      setSubmitting(true)
      const response = await api.delete(`/patients/${deletingPatient.id}`)
      ...
      toast.error(error.response?.data?.message || '删除患者失败')
    } finally {
      setSubmitting(false)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingPatient ? '修改患者信息' : '创建新的患者记录'}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button type="submit" disabled={submitting}>
                {submitting ? '提交中...' : (editingPatient ? '更新' : '创建')}
              </Button>
            </div>
          </form>
        </DialogContent>
      ...
      <AlertDialog open={!!deletingPatient} onOpenChange={() => setDeletingPatient(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>确认删除患者</AlertDialogTitle>
            <AlertDialogDescription>
              您确定要删除患者 <strong>{deletingPatient?.name}</strong> 吗？
              <br />
              <br />
              <div className="flex items-center text-red-600 text-sm">
                <AlertCircle className="h-4 w-4 mr-1" />
                此操作无法撤销，将同时删除该患者的所有检查记录！
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
